...
const ModalGestionRoles = ({ open, onClose }) => {
  ...
  const guardarRol = async () => {
    try {
      if (editando !== null) {
        await fetch(`http://66.232.105.107:3001/api/roles/${editando}`, {
          method: 'PUT',
        ...
        await fetch('http://66.232.105.107:3001/api/roles', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ nombre: nuevoRol })
        });
  ...
  const eliminarRol = async (id) => {
    try {
      await fetch(`http://66.232.105.107:3001/api/roles/${id}`, { method: 'DELETE' });
