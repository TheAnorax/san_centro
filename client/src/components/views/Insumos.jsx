...
const Insumos = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editMode) {
        await axios.put(`http://localhost:3001/api/insumos/Editar_insumo/${editId}`, formData);
        await Swal.fire("Actualizado", "El insumo fue actualizado", "success");
      } else {
        await axios.post('http://localhost:3001/api/insumos/Insertar_insumo', formData);
  ...
  const cargarInsumos = async () => {
    try {
      const res = await axios.get('http://localhost:3001/api/insumos/Obtener_insumos');
      setInsumos(res.data);
    } catch (error) {
      setInsumos([]);
  ...
  function SalidaInsumoForm({ insumo, onClose }) {
    ...
    const handleReset = async () => {
      ...
      try {
        await axios.post('http://localhost:3001/api/insumos/movimientos', {
  ...
  const handleVerMovimientos = async () => {
    ...
    try {
      const res = await axios.get('http://localhost:3001/api/insumos/Mostrar_movimientos');
