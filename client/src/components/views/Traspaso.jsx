...
function Traspaso() {
  ...
  const fetchRegistros = async () => {
    ...
    setErrorFetch('');
    try {
      const resPendientes = await axios.get('http://localhost:3007/api/RH/ObtenerTraspaso');
      const listaPendientes = resPendientes.data;

      const resRecibidos = await axios.get('http://localhost:3001/api/traspaso/recibidos');
  ...
  const handleGuardarUbicacion = async () => {
    ...
    try {
      ...
      await axios.post('http://localhost:3001/api/traspaso/guardarTraspaso', {
        Codigo: datos.Codigo,
  ...
  const recargarPendientesYMarcar = async () => {
    ...
    try {
      ...
      const resRec = await axios.get('http://localhost:3001/api/traspaso/recibidos');
